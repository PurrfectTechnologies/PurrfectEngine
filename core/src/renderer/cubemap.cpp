...
namespace PurrfectEngine {
  ...
  bool purrCubemap::initialize(std::array<purrTexture *,6> textures, VkFormat format, purrSampler *sampler) {
    ...
    if (mSampler) {
      VkDescriptorImageInfo imageInfo = getDescImageInfo();
  ...
  bool purrCubemap::initialize(glm::ivec2 size, VkFormat format, purrSampler *sampler, int mipMaps) {
    mWidth = size.x; mHeight = size.y; mSampler = sampler;
    ...
    mImage->initialize(sContext->frRenderer, fr::frImage::frImageInfo{
      ...
      (VkImageUsageFlagBits)(VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT),
      true, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
      VK_IMAGE_ASPECT_COLOR_BIT, false, static_cast<uint32_t>(mipMaps)
    });
    mImage->transitionLayout(sContext->frRenderer, transCommands, fr::frImage::frImageTransitionInfo{
      VK_IMAGE_LAYOUT_UNDEFINED,
      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
  ...
  void purrCubemap::copy(std::array<purrTexture*, 6> faces, int width, int height, int mip) {
    fr::frCommands *commands = new fr::frCommands();
    commands->initialize(sContext->frRenderer);
    commands->beginSingleTimeFrame();
    for (size_t i = 0; i < 6; ++i) mImage->copyFromImage(sContext->frRenderer, commands, faces[i]->getImage(), i, mip, width, height);
